...

{
    ...
    public function getUsers(Conversation $conversation ,$request){
        ...
        if($conversation->sender_email == $this->auth_email && auth()->guard('patient')->check() || $conversation->receiver_email == $this->auth_email && auth()->guard('patient')->check()){
            $this->receviverUser = Doctor::where('email',$conversation->receiver_email)->orWhere('email',$conversation->sender_email)->first();
        }

        if($conversation->sender_email == $this->auth_email  && auth()->guard('doctor')->check() || $conversation->receiver_email == $this->auth_email  && auth()->guard('doctor')->check()){
            $this->receviverUser = Patient::where('email',$conversation->sender_email)->orWhere('email',$conversation->receiver_email)->first();;
        }
